...
CREATE TABLE USER_REQUEST(
	...
	STATUS VARCHAR(255) references REQUEST_STATUS(CODE) NOT NULL,
	TITLE NVARCHAR2(255),
...
CREATE OR REPLACE VIEW USER_SESSIONS AS 
	SELECT REQUEST.ID AS REQUEST_ID, REQUEST.CREATE_DATE, REQUEST.USER_ID AS STUDENT_ID, USERS.FULLNAME AS STUDENT_NAME, REQUEST.TUTOR_ID, TUTOR.FULLNAME AS TUTOR_NAME, CATEGORY.TITLE AS CATEGORY, SUBJECT.TITLE AS SUBJECT, REQUEST.STATUS AS STATUS 
...
CREATE OR REPLACE VIEW SESSION_INFO AS
	SELECT REQUEST.ID AS REQUEST_ID, CATEGORY.TITLE AS CATEGORY, SUBJECT.TITLE AS SUBJECT, USERS.ID AS STUDENT_ID, USERS.FULLNAME AS STUDENT_NAME, TUTOR.ID AS TUTOR_ID, TUTOR.FULLNAME AS TUTOR_NAME, REQUEST.TITLE, REQUEST.DESCRIPTION, REQUEST.CREATE_DATE, REQUEST.UPDATE_DATE, REQUEST.PENDING_DATE, REQUEST.PROCESS_DATE, REQUEST.CLOSE_DATE, REQUEST.CANCEL_DATE
	FROM USER_REQUEST REQUEST, SUBJECT, SUBJECT_CATEGORY CATEGORY, USERS, USERS TUTOR
	WHERE SUBJECT.ID = REQUEST.SUBJECT_ID AND CATEGORY.ID = SUBJECT.CATEGORY_ID AND REQUEST.USER_ID = USERS.ID AND REQUEST.TUTOR_ID = TUTOR.ID;
...
INSERT ALL
	...
	INTO USER_REQUEST (ID, USER_ID,TUTOR_ID, SUBJECT_ID, STATUS, DESCRIPTION, CREATE_DATE, UPDATE_DATE) VALUES('3', '1', '2', '2', 'OPEN', 'Request Description', SYSDATE, SYSDATE)
SELECT * FROM DUAL;
