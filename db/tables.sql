...
CREATE OR REPLACE VIEW USER_SESSIONS AS 
	SELECT REQUEST.ID AS REQUEST_ID, REQUEST.CREATE_DATE, REQUEST.USER_ID AS STUDENT_ID, USERS.FULLNAME AS STUDENT_NAME, REQUEST.TUTOR_ID, TUTOR.FULLNAME AS TUTOR_NAME, CATEGORY.TITLE AS CATEGORY, SUBJECT.TITLE AS SUBJECT, REQUEST_STATS.TRANSACTION_STATUS AS STATUS 
	FROM USER_REQUEST REQUEST, USER_REQUEST_STATS_LOG REQUEST_STATS, SUBJECT, SUBJECT_CATEGORY CATEGORY, USERS, USERS TUTOR
	WHERE REQUEST.ID = REQUEST_STATS.USER_REQUEST_ID AND SUBJECT.ID = REQUEST.SUBJECT_ID AND CATEGORY.ID = SUBJECT.CATEGORY_ID AND REQUEST.USER_ID = USERS.ID AND REQUEST.TUTOR_ID = TUTOR.ID;
