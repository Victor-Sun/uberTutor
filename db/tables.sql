...
CREATE TABLE USER_REQUEST(
	...
	TITLE NVARCHAR2(255),
	DESCRIPTION NVARCHAR2(2000),
	USER_FEEDBACK INT references FEEDBACK(ID),
	TUTOR_FEEDBACK INT references FEEDBACK(ID),
	CREATE_DATE TIMESTAMP,
...
CREATE TABLE CHAT(
	...
	READFLAG VARCHAR2(1) DEFAULT 'N',
	RECALLED VARCHAR2(1) DEFAULT 'N'
);

-- Views
CREATE OR REPLACE VIEW V_USERS_SUBJECT_CATEGORY AS
	SELECT USER_SUBJECT.ID AS USER_SUBJECT_ID, SUBJECT.ID AS SUBJECT_ID, SUBJECT.TITLE AS SUBJECT_TITLE, SUBJECT_CATEGORY.ID AS CATEGORY_ID, SUBJECT_CATEGORY.TITLE AS CATEGORY_TITLE, USER_SUBJECT.USER_ID, USER_SUBJECT.DESCRIPTION, USER_SUBJECT.CREATE_DATE, USER_SUBJECT.UPDATE_DATE
...
CREATE OR REPLACE VIEW V_SESSION_INFO AS
	SELECT REQUEST.ID AS REQUEST_ID, CATEGORY.TITLE AS CATEGORY, SUBJECT.TITLE AS SUBJECT, STUDENT.ID AS STUDENT_ID, STUDENT.FULLNAME AS STUDENT_NAME, TUTOR.ID AS TUTOR_ID, TUTOR.FULLNAME AS TUTOR_NAME, REQUEST.TITLE, REQUEST.DESCRIPTION, REQUEST.CREATE_DATE, REQUEST.PROCESS_DATE, REQUEST.CLOSE_DATE, REQUEST.CANCEL_DATE, REQUEST.DESCRIPTION AS SUBJECT_DESCRIPTION, REQUEST.STATUS, REQUEST.TITLE AS REQUEST_TITLE	
	FROM USER_REQUEST REQUEST
	INNER JOIN USERS STUDENT
	ON REQUEST.USER_ID = STUDENT.ID
	INNER JOIN SUBJECT
...
CREATE OR REPLACE VIEW V_USER_FEEDBACK AS
	SELECT USERS.FULLNAME, USERS.ID AS USER_ID, REQUEST.TUTOR_ID, FEEDBACK.CREATE_DATE, FEEDBACK.RATING, FEEDBACK.FEEDBACK, REQUEST.USER_FEEDBACK AS FEEDBACK_ID, REQUEST.ID AS REQUEST_ID
	FROM USERS
	INNER JOIN USER_REQUEST REQUEST
	ON REQUEST.USER_ID = USERS.ID
	INNER JOIN FEEDBACK
	ON REQUEST.USER_FEEDBACK = FEEDBACK.ID;

CREATE OR REPLACE VIEW V_TUTOR_FEEDBACK AS
	SELECT USERS.FULLNAME, USERS.ID AS USER_ID, REQUEST.TUTOR_ID , FEEDBACK.CREATE_DATE, FEEDBACK.RATING, FEEDBACK.FEEDBACK, REQUEST.TUTOR_FEEDBACK AS FEEDBACK_ID, REQUEST.ID AS REQUEST_ID
	FROM USERS
	INNER JOIN USER_REQUEST REQUEST
	ON REQUEST.USER_ID = USERS.ID
	INNER JOIN FEEDBACK
	ON REQUEST.TUTOR_FEEDBACK = FEEDBACK.ID;

-- Tutor's rating
CREATE OR REPLACE VIEW V_TUTOR_RATING AS
	SELECT USER_REQUEST.USER_ID AS TUTOR_ID, AVG(FEEDBACK.RATING) AS AVERAGE_RATING
	FROM USER_REQUEST
	INNER JOIN FEEDBACK
	ON USER_REQUEST.USER_FEEDBACK = FEEDBACK.ID
	GROUP BY USER_ID;

-- User's rating
CREATE OR REPLACE VIEW V_USER_RATING AS
	SELECT USER_REQUEST.USER_ID, AVG(FEEDBACK.RATING) AS AVERAGE_RATING
	FROM USER_REQUEST
	INNER JOIN FEEDBACK
	ON USER_REQUEST.TUTOR_FEEDBACK = FEEDBACK.ID
	GROUP BY USER_ID;

CREATE OR REPLACE VIEW V_AVAILABLE_REQUESTS AS
	SELECT REQUEST.ID AS REQUEST_ID, REQUEST.CREATE_DATE, USERS.ID AS USER_ID, USERS.FULLNAME AS USER_FULLNAME, TUTOR_SUBJECT.USER_ID AS TUTOR_ID, CATEGORY.TITLE AS CATEGORY, SUBJECT.TITLE AS SUBJECT_TITLE, REQUEST.STATUS, SUBJECT.DESCRIPTION AS SUBJECT_DESCRIPTION, REQUEST.TITLE AS REQUEST_TITLE, RATING.AVERAGE_RATING
	FROM USER_SUBJECT TUTOR_SUBJECT
	LEFT OUTER JOIN USER_REQUEST REQUEST
	ON TUTOR_SUBJECT.SUBJECT_ID = REQUEST.SUBJECT_ID AND REQUEST.STATUS = 'OPEN'
	INNER JOIN SUBJECT
	ON SUBJECT.ID = TUTOR_SUBJECT.ID
	INNER JOIN SUBJECT_CATEGORY CATEGORY
	ON CATEGORY.ID = SUBJECT.CATEGORY_ID
	INNER JOIN USERS
	ON USERS.ID = REQUEST.USER_ID
	LEFT OUTER JOIN V_USER_RATING RATING
	ON RATING.USER_ID = USERS.ID;

-- TO BE CHANGED
-- CREATE OR REPLACE VIEW V_AVERAGE_RATING AS
-- 	SELECT TUTOR.ID AS TUTOR_ID, TUTOR.FULLNAME AS TUTOR_FULLNAME, AVG(RATING) AS AVERAGE_RATING
-- 	FROM FEEDBACK, USER_REQUEST, USERS TUTOR
-- 	WHERE FEEDBACK.ID = USER_REQUEST.FEEDBACK AND TUTOR.ID = USER_REQUEST.TUTOR_ID
-- 	GROUP BY TUTOR.FULLNAME, TUTOR.ID;

CREATE OR REPLACE VIEW V_PREVIOUS_TUTOR AS
	SELECT REQUEST.ID AS REQUEST_ID, TUTOR.ID AS TUTOR_ID, TUTOR.FULLNAME AS TUTOR_FULLNAME, USERS.ID AS USER_ID, SUBJECT.TITLE AS SUBJECT_TITLE, REQUEST.CREATE_DATE
	FROM USER_REQUEST REQUEST, USERS TUTOR, USERS, SUBJECT
	WHERE REQUEST.TUTOR_ID = TUTOR.ID AND REQUEST.USER_ID = USERS.ID AND REQUEST.SUBJECT_ID = SUBJECT.ID;
...
INSERT ALL
	INTO FEEDBACK (ID, CREATE_DATE, RATING, FEEDBACK) VALUES ('1', SYSDATE, 5, 'This was a great tutor!')
	INTO FEEDBACK (ID, CREATE_DATE, RATING, FEEDBACK) VALUES ('2', SYSDATE, 4, 'This was a okay tutor!')
SELECT * FROM DUAL;

INSERT ALL
	INTO USER_REQUEST (ID, USER_ID, TUTOR_ID, SUBJECT_ID, STATUS, DESCRIPTION, USER_FEEDBACK, TUTOR_FEEDBACK, CREATE_DATE, CLOSE_DATE) VALUES('1', '1', '2', '1', 'CLOSED', 'Request Description', '1', '2', SYSDATE, SYSDATE)
