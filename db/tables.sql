...
CREATE OR REPLACE VIEW USERS_SUBJECT_CATEGORY AS
	SELECT USER_SUBJECT.ID AS ID, SUBJECT.ID AS SUBJECT_ID, SUBJECT.TITLE AS SUBJECT_TITLE, SUBJECT_CATEGORY.ID AS CATEGORY_ID, SUBJECT_CATEGORY.TITLE AS CATEGORY_TITLE, USER_SUBJECT.USER_ID
...
CREATE OR REPLACE VIEW USER_SESSIONS AS 
	SELECT REQUEST.ID AS REQUEST_ID, REQUEST.CREATE_DATE, REQUEST.USER_ID AS STUDENT_ID, USERS.FULLNAME AS STUDENT_NAME, REQUEST.TUTOR_ID, TUTOR.FULLNAME AS TUTOR_NAME, CATEGORY.TITLE AS CATEGORY, SUBJECT.TITLE AS SUBJECT, REQUEST.STATUS AS STATUS, REQUEST.DESCRIPTION AS SUBJECT_DESCRIPTION
	FROM USER_REQUEST REQUEST, SUBJECT, SUBJECT_CATEGORY CATEGORY, USERS, USERS TUTOR
	WHERE SUBJECT.ID = REQUEST.SUBJECT_ID AND CATEGORY.ID = SUBJECT.CATEGORY_ID AND REQUEST.USER_ID = USERS.ID AND REQUEST.TUTOR_ID = TUTOR.ID;

CREATE OR REPLACE VIEW SESSION_INFO AS
	SELECT REQUEST.ID AS REQUEST_ID, CATEGORY.TITLE AS CATEGORY, SUBJECT.TITLE AS SUBJECT, USERS.ID AS STUDENT_ID, USERS.FULLNAME AS STUDENT_NAME, TUTOR.ID AS TUTOR_ID, TUTOR.FULLNAME AS TUTOR_NAME, REQUEST.TITLE, REQUEST.DESCRIPTION, REQUEST.CREATE_DATE, REQUEST.UPDATE_DATE, REQUEST.PENDING_DATE, REQUEST.PROCESS_DATE, REQUEST.CLOSE_DATE, REQUEST.CANCEL_DATE, REQUEST.DESCRIPTION AS SUBJECT_DESCRIPTION, REQUEST.STATUS
	FROM USER_REQUEST REQUEST, SUBJECT, SUBJECT_CATEGORY CATEGORY, USERS, USERS TUTOR
	WHERE SUBJECT.ID = REQUEST.SUBJECT_ID AND CATEGORY.ID = SUBJECT.CATEGORY_ID AND REQUEST.USER_ID = USERS.ID AND REQUEST.TUTOR_ID = TUTOR.ID;
...
INSERT ALL
	INTO USER_REQUEST (ID, USER_ID, TUTOR_ID, SUBJECT_ID, STATUS, DESCRIPTION, CREATE_DATE, UPDATE_DATE) VALUES('1', '1', '2', '1', 'CLOSED', 'Request Description', SYSDATE, SYSDATE)
	INTO USER_REQUEST (ID, USER_ID, TUTOR_ID, SUBJECT_ID, STATUS, DESCRIPTION, CREATE_DATE, UPDATE_DATE) VALUES('2', '1', '2', '2', 'CLOSED', 'Request Description', SYSDATE, SYSDATE)
	INTO USER_REQUEST (ID, USER_ID, SUBJECT_ID, STATUS, DESCRIPTION, CREATE_DATE, UPDATE_DATE) VALUES('3', '1', '2', 'OPEN', 'Request Description', SYSDATE, SYSDATE)
SELECT * FROM DUAL;
