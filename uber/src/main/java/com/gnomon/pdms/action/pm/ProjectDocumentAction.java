...
@Namespace("/pm")
public class ProjectDocumentAction extends PDMSCrudActionSupport<ProgramEntity> {
	...
	public void getDocumentTree() {
		...
		if (PDMSCommon.isNotNull(node) && node.equals("root")) {
			...
			dataMap.put("text", "项目文档查询");
			dataMap.put("iconCls", "x-fa fa-search");
			dataMap.put("viewType", "projectDocumentList");
			dataMap.put("expanded", true);
			data.add(dataMap);
			// 用户文件夹
			dataMap = new HashMap<String,Object>();
			dataMap.put("id", this.documentService.getCusomizeFolderId(programId));
			dataMap.put("text", "用户文档");
